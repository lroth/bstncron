...
class ctwitter_stream
{
    ...
    private function process_tweet(array $_data)
    {
        try {
			...
			$twitter_user_id = $_data['user']['id'];
			$twitter_name = $_data['user']['name'];
			$twitter_screen_name = $_data['user']['screen_name'];
			
			$twitter_msg = $_data['text'];
			
			$vote = null;
			
			foreach ($this->_vote_keywords as $keyword) {
				if ( stristr($twitter_msg, $keyword) ) {
					$vote = $keyword;
					break;
				}
			}
			...
			if ($vote !=  null ) {
	        	if (!isset($this->_dbh) ) {
					$this->_dbh = new PDO("mysql:host=" . $this->_db_host . ";dbname=" . $this->_db_name, $this->_db_user, $this->_db_pass);
	        	}
				
				$this->_dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$this->_dbh->exec("SET NAMES 'UTF8'");
	
				$sql = $this->_dbh->prepare("REPLACE INTO battle_of_the_beer (twitter_user_id, twitter_name, twitter_screen_name, hashtag, twitter_msg) VALUES (?,?,?,?,?)");
				$sql->execute( array( $twitter_user_id, $twitter_name, $twitter_screen_name,$vote, $twitter_msg ) );
	
				//output msg
				print ( date("d.m.Y H:m:s") . " TWITTER -VOTE FROM: ". $twitter_screen_name . " (" .  $twitter_name . ") - " .  $twitter_msg . "\n\r" );				
			}
    ...
    //
    public function start(array $_keywords, $vote_keywords)
    {
        $this->_vote_keywords = $vote_keywords;
